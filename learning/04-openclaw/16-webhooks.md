...
```typescript
this.api = new apigateway.HttpApi(this, 'OpenClawApi', {
  corsPreflight: {
...
Create `ui/openclaw-chat.js`. This file defines a custom HTML element `<openclaw-chat>`.

**Key Feature:** The `attachShadow({ mode: 'open' })` line creates a private playground for our CSS.

```javascript
class OpenClawChat extends HTMLElement {
    constructor() {
        super();
    ...
    connectedCallback() {
        ...
        const sessionId = localStorage.getItem('openclaw_sid') || Math.random().toString(36).substring(7);
        localStorage.setItem('openclaw_sid', sessionId);
...
customElements.define('openclaw-chat', OpenClawChat);
```
...
<script src="path/to/openclaw-chat.js"></script>

<!-- 2. Use the tag -->
<openclaw-chat url="https://YOUR_API_GATEWAY_URL/webhook"></openclaw-chat>
```
